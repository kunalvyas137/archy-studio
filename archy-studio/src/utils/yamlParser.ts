...
export function parseArchyYaml(yamlString: string): ParseResult {
  ...
  if (rootFlow.tasks) {
    rootFlow.tasks.forEach((tWrapper: any) => {
      ...
      if (task.actions && Array.isArray(task.actions)) {
        const { firstNodeId } = processActions(
          task.actions,
          taskId,
          nodes,
          edges,
  ...
  // Handle startUpTaskActions (commonModule)
  ...
  // Second pass: Connect jumpToTask / callTask / jumpToMenu
  nodes.forEach((node) => {
    if (
      node.data.type === 'jumpToTask' ||
      node.data.type === 'callTask' ||
      node.data.type === 'jumpToMenu'
    ) {
      const targetRef =
        node.data.details.targetTaskRef || node.data.details.targetMenuRef;
      ...
      if (targetRef && taskRefMap[targetRef]) {
        ...
        const foundKey = Object.keys(taskRefMap).find(
          (k) => targetRef && targetRef.includes(k)
        );
        if (foundKey) {
          foundTarget = true;
          edges.push({
      ...
      if (!foundTarget && targetRef) {
        ...
        node.data.validationWarnings.push(
          `Unresolved Reference: Could not find target '${targetRef}'.`
        );
      }
    }
  });
  ...
  if (rootFlow.settingsErrorHandling?.errorHandling?.task?.targetTaskRef) {
    const errorTaskId =
      taskRefMap[rootFlow.settingsErrorHandling.errorHandling.task.targetTaskRef];
    if (errorTaskId) {
...
const TERMINATOR_TYPES = new Set(['disconnect', 'jumpToTask', 'jumpToMenu', 'endTask']);
...
function processActions(
  ...
  actions.forEach((actionWrapper, index) => {
    ...
    if (dependencyActions.includes(actionType)) {
      ...
      if (!hasFailure) {
        warnings.push(
          `Missing Fallback: ${actionType} does not have a configured failure or timeout path.`
        );
      }
    }
    ...
    const isSwitchAction = actionType === 'switch' && action.evaluate?.firstTrue?.cases;
    const hasGenericBranches = GENERIC_BRANCH_KEYS.some(
      (key) => action[key]?.actions && action[key].actions.length > 0
    );
    ...
    if (isSwitchAction) {
      action.evaluate.firstTrue.cases.forEach((cWrapper: any, i: number) => {
        ...
        if (c.actions && c.actions.length > 0) {
          const { firstNodeId: caseFirst, lastNodeIds: caseLast } = processActions(
            c.actions,
            actionId,
            nodes,
            edges,
          ...
          if (caseFirst) {
            edges.push({ id: generateEdgeId(), source: actionId, target: caseFirst, label, type: 'smoothstep' });
          }
      ...
      if (action.evaluate.firstTrue.default?.actions) {
        const { firstNodeId: defFirst, lastNodeIds: defLast } = processActions(
          action.evaluate.firstTrue.default.actions,
          actionId,
          nodes,
          edges,
          generateId,
        ...
        if (defFirst) {
          edges.push({ id: generateEdgeId(), source: actionId, target: defFirst, label: 'default', type: 'smoothstep' });
        }
    ...
    if (hasGenericBranches) {
      GENERIC_BRANCH_KEYS.forEach((branchKey) => {
        ...
        if (branchObj && branchObj.actions && branchObj.actions.length > 0) {
          const { firstNodeId: outFirst, lastNodeIds: outLast } = processActions(
            branchObj.actions,
            actionId,
            nodes,
            edges,
          ...
          if (outFirst) {
            edges.push({ id: generateEdgeId(), source: actionId, target: outFirst, label: branchKey, type: 'smoothstep' });
          }
